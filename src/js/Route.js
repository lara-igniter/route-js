...
/**
 * A Laraigniter route. This class represents one route and its configuration and metadata.
 */
export default class Route {
     ...
     *
     * @example
     * https://{team}.example.dev/user/{user}
     *
    ...
    compile(params) {
        ...
        return this.template.replace(/{([^}?]+)(\??)}/g, (_, segment, optional) => {
            ...
            if (!optional && [null, undefined].includes(params[segment])) {
                throw new Error(`RouteJS error: '${segment}' parameter is required for route '${this.name}'.`)
            }
            ...
            if (this.wheres[segment] && !new RegExp(`^${optional ? `(${this.wheres[segment]})?` : this.wheres[segment]}$`).test(params[segment] ?? '')) {
                throw new Error(`RouteJS error: '${segment}' parameter does not match required format '${this.wheres[segment]}' for route '${this.name}'.`)
            }
